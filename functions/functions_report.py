    ...
    def generate_html_report(self):
        try:
            ...
            html = template.render(
                ...
                status=status,
                suppliers_processed=len(self.stats['suppliers_processed']),
                platforms_processed=len(self.stats['platforms_processed']),
                files_successful=len(self.stats['files_successful']),
                files_failed=len(self.stats['files_failed']),
                products_updated=self.stats['products_updated'],
                duration=duration,
                errors=self.stats['errors'],
                warnings=self.stats['warnings'],
                files_successful_list=self.stats['files_successful'],
                files_failed_list=self.stats['files_failed']
            )
    ...
    def send_email_report(self, recipients: List[str]):
        try:
            ...
            if not smtp_email or not smtp_password:
                self.logger.error("Identifiants SMTP manquants dans les variables d'environnement.")
                return False
