...
def read_fournisseur(data_f):
    ...
    #print('\n\n data: ', df_f_info['dataset'].head())
    #print('This', df_f_info['dataset'][df_f_info['dataset'][nom_reference_f]== 'BM91518H'])#[df[nom_ref] == 45312])

    
    df_f = df_f_info['dataset'].copy()  # df

    #print(df_f_info['dataset'].head())
    #print(df_f[nom_reference_f].dtype)
    

    #print('This is row original', df_f_info['dataset'][df_f_info['dataset'][nom_reference_f]== 'BM91518H'])#[df[nom_ref] == 45312])
    
    #print('This is reduced qte row original processed', df_f_info['dataset'][df_f_info['dataset'][nom_reference_f] == 'BM91518H'])


    df_f[quantite_stock_f] = df_f[quantite_stock_f].apply(process_stock_value)   # df[nom_qte]
    #print('This is row original processed', df_f[df_f[nom_reference_f] == 'BM91518H'])
    #print('*** *** ', df_f.head())


    reduced_cols_df = df_f[[nom_reference_f, quantite_stock_f]].copy()
    reduced_cols_df[quantite_stock_f] = reduced_cols_df[quantite_stock_f].astype(int)
    
    reduced_cols_df.columns = [ID_PRODUCT, QUANTITY]
    #print('Fournisseur:', chemin_fichier_f, '   -    ref',nom_reference_f ,'    -    Qte', quantite_stock_f)
    #print('This is reduced row original processed', reduced_cols_df[reduced_cols_df[ID_PRODUCT] == 'BM91518H'])

    return {
        'Chemin': chemin_fichier_f,
        'ref': nom_reference_f,
        'qte': quantite_stock_f,
